...
export function DeviceGlow(props: DeviceGlowProps): JSX.Element {
  ...
  useEffect(() => {
    ...
    // The GL setup waits for the opening to be playing (lib/launch-gate.ts):
    // at a cold start the rail's synchronous GPU setup ran before the
    // opening's first frame, behind the splash that hides this card. The
    // opening's run is this card's time to start; the art shows unlit until.
    ...
    const cancel = afterLaunch("opening", () => {
      teardown = setUp();
    });
    return () => {
      cancel();
