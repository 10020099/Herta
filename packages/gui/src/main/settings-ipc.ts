...
export function registerSettingsHandlers(deps: SettingsIpcDeps): void {
  ...
  // Settings → Context: the automatic recap threshold. Restart-to-apply, same
  // contract as the rows above — a session's recap runtime is immutable once
  // constructed, while subsequent sessions read the saved level.
  ...
  handle(
    ...
    () => deps.host()?.activeSession?.getMcpConnectionStatus?.() ?? {},
  );
  ...
  handle(CMD.deleteProjectRule, async (_e, name: unknown) => {
    ...
    try {
      rmSync(join(projectRulesWorkspace(), ".herta", name), { force: true });
      return { ok: true };
    } catch {
      return { ok: false, message: "could not delete project rule" };
    }
  });
  // Settings → Multi-provider support. The active provider decides which
  // stored key and model overrides buildConfig uses at the next bootstrap.
  handle(CMD.getActiveProvider, () => {
    const s = readAppSettingsSync(workspaceRoot());
    return (s.activeProvider ?? "deepseek") as ProviderType;
  });
  handle(CMD.setActiveProvider, async (_e, type: ProviderType) => {
    await updateAppSettings(workspaceRoot(), { activeProvider: type });
  });
